...
(function (factory) {
    ...
    , createToken: function (attrs, triggerChange) {
        ...
        //judge  attributes length
        if (createEvent.attrs.value.length > 60) return
        var $token = $('<div class="token" />')

              .append('<span class="token-label" />')
            .append('<input type="hidden"  name="tagvalue" value="' + attrs.value + '"/>')
    ...
    , keydown: function (e) {
        ...
        var isentry = false;
        try {
            if (this.$element.data("isentry") == "True") {
                isentry = true;
            }
        } catch (e) { }
        switch (e.keyCode) {
